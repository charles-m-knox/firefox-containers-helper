 ...
 */
export const del = async (contexts: Container[], prompt = true): Promise<number> => {
 ...
 */
export const setUrls = async (contexts: Container[], url: string[], allowAnyProtocol = false, updateHelp = true) => {
 ...
 */
export const setUrlsPrompt = async (contexts: Container[]) => {
 ...
 */
export const rename = async (contexts: Container[]) => {
  const one = contexts.length === 1;
 ...
 */
export const update = async (contexts: Container[], key: string, value: string) => {
  const updated: Container[] = [];
...
export const setColors = async (contexts: Container[]) => {
  const one = contexts.length === 1;
  const s = one ? '' : 's';
...
export const setIcons = async (contexts: Container[]) => {
  const one = contexts.length === 1;
  const s = one ? '' : 's';
 ...
 */
export const replaceInName = async (contexts: Container[]) => {
  const one = contexts.length === 1;
  const s = one ? '' : 's';
 ...
 * @param contexts The `contextualIdentities` to change.
 */
export const replaceInUrls = async (contexts: Container[]) => {
  const one = contexts.length === 1;
  const s = one ? '' : 's';
 ...
 * @param prompt If false, no modals are shown.
 */
export const duplicate = async (contexts: Container[], prompt = true): Promise<number> => {
 ...
 */
export const refresh = async (contexts: Container[]) => {
  const s = contexts.length === 1 ? '' : 's';
...
export const getActionable = (
  ...
  if (isAnyContextSelected(selected)) {
    ...
    if (shiftModifier) {
      ...
      if (clicked) {
        contexts.push(clicked);
      } else {
        contexts.push(filtered[0]);
      }
    }
  }
...
const applyQuery = async (contexts: Container[], queryLower: string) => {
...
const reflectFiltered = async (results: Container[], actualTabUrl?: string | null) => {
  ...
  results.map(async (context, i) =>
    ul.appendChild(await buildContainerListItem(results, context, i, tab, actualTabUrl || '', mode, actHandler)),
  );
